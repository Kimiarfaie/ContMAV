...
from palette import get_cityscapes_palette, colorize_mask, get_liaci_palette
from utils.transforms import get_restore_transform, get_viz_transform
...
def train_main():
    ...
    # Choose the palette based on the dataset
    if args.dataset == "cityscapes":
        palette = get_cityscapes_palette()
    elif args.dataset == "liaci":
        palette = get_liaci_palette()
    else:
    ...
    train_loader, valid_loader, test_loader = data_loaders # why not getting the test data?
    
    #should change this later for LIACi
    # palette = get_cityscapes_palette()

    n_classes_without_void = train_loader.dataset.n_classes_without_void
    if args.class_weighting != "None": # use class weighting for dataset imbalance
    ...
    print('num of class for model: ', n_classes_without_void)
    # model building -----------------------------------------------------------
    model, device = build_model(args, n_classes=n_classes_without_void)

    # add things for parallel computing
    ...
    lr_scheduler = OneCycleLR(
        ...
        steps_per_epoch=len(train_loader), #this is an addition to avoid zero division in pct
    ...
    writer = SummaryWriter("runs/" + args.dataset + ckpt_dir.split(args.dataset)[-1])

    # to testing mode
    if args.code_mode == 'test_close':

        checkpoint_path = args.checkpoint_path # change this for different model
        ...
        # new summary writer
        writer = SummaryWriter(os.path.join("runs/test", args.dataset, args.code_mode, args.test_notes))

        miou = test_closed_world(
                    model,
                    test_loader,
                    device,
                    val_loss,
...
def train_one_epoch(
    ...
    # Kimia Added this, Log the learning rate at the start of each epoch
    writer.add_scalar("Learning_Rate", optimizer.param_groups[0]["lr"], epoch)
    # Conditionally log weight decay for Adam and momentum for SGD
    if isinstance(optimizer, torch.optim.Adam):
        writer.add_scalar("Weight_Decay", optimizer.param_groups[0]["weight_decay"], epoch)
    elif isinstance(optimizer, torch.optim.SGD):
        writer.add_scalar("Momentum", optimizer.param_groups[0]["momentum"], epoch)
    ...
    for i, sample in enumerate(train_loader):
        ...
        label = sample["label"].long().cuda() - 1 s
        label[label < 0] = 255
...
def validate(
    ...
    for i, sample in enumerate(tqdm(valid_loader, desc="Valid step")):
        ...
        with torch.no_grad():
            ...
            output_np = prediction_ss.data.max(1)[1].cpu().numpy()  # Convert predictions to label map
            val_visual.append([sample['image'][0].cpu(), target_np[0], output_np[0]])
    ...
    for d, t, o in val_visual:
        # Restore images for visualization
        restore_transform = get_restore_transform(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) 
        d = restore_transform(d)  
        t, o = colorize_mask(t, palette), colorize_mask(o, palette)  
        d, t, o = d.convert('RGB'), t.convert('RGB'), o.convert('RGB')
        viz_transform = get_viz_transform()
        [d, t, o] = [viz_transform(x) for x in [d, t, o]]
        val_img.extend([d, t, o])

    # Stack images into grid and add to TensorBoard
    val_img = torch.stack(val_img, 0)
    val_img = make_grid(val_img.cpu(), nrow=3, padding=5)
    writer.add_image(f'val/inputs_targets_predictions', val_img, epoch)
...
def test_ow(
    ...
    # CHANGE THE MAVS AND VARS PATH HERE
    with open("/home/colourlabgpu4/Kimia/OWRepo/ContMAV/mavs_vars_LIACi_OW_NO_NORMALIZATION /mavs.pickle", "rb") as h1:
        mavs = pickle.load(h1)
    with open("/home/colourlabgpu4/Kimia/OWRepo/ContMAV/mavs_vars_LIACi_OW_NO_NORMALIZATION /mavs.pickle", "rb") as h2:
        vars = pickle.load(h2)
    ...
    ious = compute_iou.compute().detach().cpu()
    writer.add_scalar("Metrics/OWS/known", ious[0])
    writer.add_scalar("Metrics/OWS/unknown", ious[1])
...
def test_closed_world(
    ...
    for i, sample in enumerate(tqdm(test_loader, desc="Test step")):
        ...
        with torch.no_grad():
            ...
            # Compute loss
            loss_function_valid.add_loss_of_batch(prediction_ss, label)

            # Compute IoU for each class
            compute_iou.update(prediction_ss, target.cuda())
            iou_per_class = compute_iou.compute().detach().cpu().numpy()
            mean_iou = iou_per_class.mean()

            # Store the best and worst samples
            target_np = target.cpu().numpy()
            output_np = prediction_ss.data.max(1)[1].cpu().numpy()

            # Store top 5 best and worst samples
            if len(best_ious) < 5 or mean_iou > min(best_ious):
                if len(best_ious) == 5:
                    min_idx = best_ious.index(min(best_ious))
                    del best_ious[min_idx]
                    del best_samples[min_idx]
                best_ious.append(mean_iou)
                best_samples.append([sample['image'][0].cpu(), target_np[0], output_np[0]])

            if len(worst_ious) < 5 or mean_iou < max(worst_ious):
                if len(worst_ious) == 5:
                    max_idx = worst_ious.index(max(worst_ious))
                    del worst_ious[max_idx]
                    del worst_samples[max_idx]
                worst_ious.append(mean_iou)
                worst_samples.append([sample['image'][0].cpu(), target_np[0], output_np[0]])

            # Add to confusion matrix
            all_preds.extend(output_np.flatten())
            all_labels.extend(target_np.flatten())

        # Visualization of all images (immediately after processing each batch)
        all_images = []
        for d, t, o in zip(sample['image'], target_np, output_np):
            restore_transform = get_restore_transform(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) 
            d = restore_transform(d)
            t, o = colorize_mask(t, o, palette)
            d, t, o = d.convert('RGB'), t.convert('RGB'), o.convert('RGB')
            viz_transform = get_viz_transform()
            [d, t, o] = [viz_transform(x) for x in [d, t, o]]
            all_images.extend([d, t, o])
            del d, t, o

        all_images_grid = make_grid(all_images, nrow=3, padding=5)
        writer.add_image(f'test/inputs_targets_predictions_batch_{i}', all_images_grid)

    total_loss = loss_function_valid.compute_whole_loss()

    # Compute IoU for known classes
    ious = compute_iou.compute().detach().cpu().numpy()
    miou = ious.mean()

    writer.add_scalar("Loss/test", total_loss / len(test_loader))
    writer.add_scalar("Metrics/miou", miou)
    for i in range(classes):
        writer.add_scalar(f"Class_metrics/iou_{i}", ious[i])

    best_img = []
    for d, t, o in best_samples:
        # Restore images for visualization
        restore_transform = get_restore_transform(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) 
        d = restore_transform(d)  
        t, o = colorize_mask(t, o, palette) 
        ...
        viz_transform = get_viz_transform()
        [d, t, o] = [viz_transform(x) for x in [d, t, o]]  
        best_img.extend([d, t, o])
        del d, t, o
    # Stack images into grid and add to TensorBoard
    best_img = torch.stack(best_img, 0)
    best_img = make_grid(best_img.cpu(), nrow=3, padding=5)  
    writer.add_image(f'val/best_samples', best_img)

    worst_img = []
    for d, t, o in worst_samples:
        # Restore images for visualization
        restore_transform = get_restore_transform(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) 
        d = restore_transform(d)  
        t, o = colorize_mask(t, o, palette)
        ...
        viz_transform = get_viz_transform()
        [d, t, o] = [viz_transform(x) for x in [d, t, o]]
        worst_img.extend([d, t, o])
        del d, t, o
    # Stack images into grid and add to TensorBoard
    worst_img = torch.stack(worst_img, 0)
    worst_img = make_grid(worst_img.cpu(), nrow=3, padding=5)
    writer.add_image(f'val/worst_samples', worst_img)

    # Plot IoU for all classes as a bar graph
    plot_class_iou_bar_chart(ious, writer)

    # Confusion Matrix
    cm = confusion_matrix(all_labels, all_preds, labels=list(range(classes)))
    row_sums = cm.sum(axis=1, keepdims=True)  # Compute row sums
    cm_norm = np.divide(cm.astype('float'), row_sums, out=np.zeros_like(cm, dtype=float), where=row_sums != 0)
    plot_confusion_matrix(cm_norm, writer, classes)

    # Save confusion matrix as a table
    class_names = [f"Class_{i}" for i in range(classes)]
    cm_df = pd.DataFrame(cm, index=class_names, columns=class_names)
    cm_file_path = os.path.join(writer.get_logdir(), 'confusion_matrix.csv')
    cm_df.to_csv(cm_file_path)

    return miou

def plot_class_iou_bar_chart(ious, writer):
    plt.figure(figsize=(10, 6))
    plt.bar(range(len(ious)), ious, color="blue")
    plt.xlabel("Class")
    plt.ylabel("IoU")
    plt.title("IoU for Each Class")
    plt.xticks(range(len(ious)))
    plt.tight_layout()
    bar_chart_path = os.path.join(writer.get_logdir(), "class_iou_bar_chart.png")
    plt.savefig(bar_chart_path)
    plt.close()

def plot_confusion_matrix(cm, writer, classes):
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt=".2f", cmap="Blues", xticklabels=list(range(classes)), yticklabels=list(range(classes)))
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.title('Confusion Matrix')
    confusion_matrix_path = os.path.join(writer.get_logdir(), 'confusion_matrix.png')
